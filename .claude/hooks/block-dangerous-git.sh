...
if ! command -v jq >/dev/null 2>&1; then
  echo "BLOCKED: jq is required for guardrail hook." >&2
  exit 2
fi
if ! COMMAND=$(echo "$INPUT" | jq -er '.tool_input.command // empty'); then
  echo "BLOCKED: invalid hook payload; cannot parse tool_input.command." >&2
  exit 2
fi
