...
**通过 subagent 调用**第三层 `/lp-dev <N>`，避免污染 lp-mr 的上下文：
...
```text
[开始] → /lp-dev → commit+push+mr create
    → watch-pr
...
当 worktree 缺少状态文件时，从 GitHub PR 推断：
...
```bash
# 检查本地状态文件
STATUS_FILE=".claude/worktrees/issue-<N>/.status"
FIX_ROUND_FILE=".claude/worktrees/issue-<N>/.fix_round"
CLOSE_REOPEN_FILE=".claude/worktrees/issue-<N>/.close_reopen_count"

if [ -f "$STATUS_FILE" ]; then
  status=$(cat "$STATUS_FILE")
else
  # 从 GitHub 推断
  status=$(infer_status_from_github <issue-number>)
  [ -n "$status" ] && echo "$status" > "$STATUS_FILE"
fi

# 恢复计数器
if [ -f "$FIX_ROUND_FILE" ]; then
  fix_round=$(cat "$FIX_ROUND_FILE")
else
  fix_round=$(infer_fix_round_from_github <pr-number>)
  echo "$fix_round" > "$FIX_ROUND_FILE"
fi

if [ -f "$CLOSE_REOPEN_FILE" ]; then
  close_reopen_count=$(cat "$CLOSE_REOPEN_FILE")
else
  close_reopen_count=$(infer_close_reopen_from_github <pr-number>)
  echo "$close_reopen_count" > "$CLOSE_REOPEN_FILE"
fi
