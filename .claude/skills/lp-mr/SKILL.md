---
name: lp-mr
description: 第三层飞轮——MR 全生命周期管理：提交、创建、监控、分配修复
---
...
# 2. 获取 PR 状态
PR_STATE=$(gh pr view "$PR_NUMBER" --repo Qanora/alpha-screener --json state,statusCheckRollup --jq '{state, checks: [.statusCheckRollup[] | select(.status == "COMPLETED" and .conclusion == "FAILURE")]}')
