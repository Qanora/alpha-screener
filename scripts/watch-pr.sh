...
# Exit: 0=merged, 1=CI failure, 2=stuck/timeout, 4=changes requested, 5=missing tools
set -euo pipefail
...
while true; do
  ...
  # Action needed: changes requested (CodeRabbit wants changes)
  if [ "$REVIEW" = "CHANGES_REQUESTED" ]; then
    echo "=== CHANGES_REQUESTED — run fetch-review.sh $PR to get comments ==="
    exit 4
  fi
  ...
  # Developer should decide whether to fix. watch-pr continues polling.
