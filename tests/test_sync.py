...
def test_default_universe_combines_official_directories_and_adds_spy(tmp_path, monkeypatch) -> None:
    created = datetime.now(ZoneInfo("America/New_York")).strftime("%m%d%Y%H:%M")
    nasdaq = "\n".join(
        [
            "Symbol|Security Name|Market Category|Test Issue|Financial Status|"
            "Round Lot Size|ETF|NextShares",
    ...
    assert len(list((tmp_path / "data" / "universe").glob("as_of=*/digest=*"))) == 1
...
def test_partial_sync_reports_coverage_and_preserves_failed_ticker_history(
    ...
    def download(tickers, **kwargs):
        ...
        dates = pd.to_datetime(market_dates_between(date(2025, 1, 2), date(2025, 6, 30))[:76])
        return pd.DataFrame(
            {
                "Open": [10.0] * 76,
                "High": [11.0] * 76,
...
def test_download_end_includes_a_market_session_completed_today(tmp_path, monkeypatch) -> None:
    monkeypatch.setattr("alphascreener.data.paths.get_data_home", lambda: tmp_path)
    monkeypatch.setattr(
...
def test_fresh_backtest_ready_universe_skips_redundant_downloads(tmp_path, monkeypatch) -> None:
    monkeypatch.setattr("alphascreener.data.paths.get_data_home", lambda: tmp_path)
    rows = []
...
def test_fresh_spy_gap_forces_a_repair_download_even_above_90pct_coverage(
    ...
    rows = [row for ticker in tickers for row in _stored_rows(ticker, 118)]
    spy_dates = [row["dt"] for row in rows if row["ticker"] == "SPY"]
    missing_date = spy_dates[-30]
    rows = [row for row in rows if not (row["ticker"] == "SPY" and row["dt"] == missing_date)]
    write_ohlcv(pl.DataFrame(rows))
    calls = []
    monkeypatch.setattr(
        "alphascreener.data.sync._download_batch",
...
def test_fresh_prediction_ready_cache_is_expanded_for_backtests(tmp_path, monkeypatch) -> None:
    monkeypatch.setattr("alphascreener.data.paths.get_data_home", lambda: tmp_path)
    write_ohlcv(
        pl.DataFrame([row for ticker in ("SPY", "GOOD") for row in _stored_rows(ticker, 76)])
    )
    calls = []
...
def test_prediction_ready_ipo_is_not_excluded_by_backtest_history_requirement(
    ...
    rows = [row for ticker in mature_tickers for row in _stored_rows(ticker, 118)]
