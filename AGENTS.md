...
Alpha Screener 是面向美股的 AI 原生量化策略验证平台。核心能力包括数据获取、因子计算、股票筛选、参数优化、回测、数据库存储和命令行操作。

- Python 3.11+；依赖与工具配置以 `pyproject.toml` 为准。
- 业务代码在 `alphascreener/`，测试在 `tests/`，数据库迁移在 `alembic/`。
